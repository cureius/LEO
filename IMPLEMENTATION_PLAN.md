...
| 2026-05-12 | TodayView has list mode (default) and timeline mode (toggle in header) | Timeline is a 24-hour scrollable hour grid using VStack-for-hour-rows + overlay GeometryReader for event blocks. ZStack+offset was tried first but breaks ScrollViewReader.scrollTo — VStack gives real layout positions. |
| 2026-05-12 | Recurring EKReminders expanded into individual occurrences at sync time | EKReminder returns one object per series; `expandRecurringReminder()` in EventKitBridge generates one item per occurrence in the ±30-day window. Occurrence IDs: `baseID/yyyyMMddTHHmm`. Deletion pass extracts base ID (prefix before "/") to detect orphans. |
| 2026-05-12 | Recurring items created in LEO expanded to individual items at save time | `ItemDetailViewModel.saveRecurring()` calls `RecurrenceEngine.shared.occurrences()` for 1 year, then `ItemRepository.addBatch()` to save all occurrences in one SwiftData transaction. Previously `recurrenceRule` was collected by the UI but silently discarded in `buildItem()`. |
| 2026-05-13 | `rruleRaw: String?` added to Item protocol + 4 concrete types + 4 stored models (SchemaV3) | Each occurrence now carries its RFC 5545 rule string so the UI can show frequency. Lightweight migration V1→V3 (skip V2 to avoid duplicate-checksum crash — SwiftData computes checksums from live class definitions, not version identifiers). `populate()` reads it back so ItemDetailSheet shows the correct recurrence. |
| 2026-05-13 | SwiftData migration plan must skip intermediate versions when model classes are shared | SchemaV2+V3 both reference the same (updated) model classes → identical checksums → "Duplicate version checksums detected" crash at ModelContainer init. Fix: plan uses `[SchemaV1, SchemaV3]` only; V2 stores auto-migrate (additive-only changes). |
